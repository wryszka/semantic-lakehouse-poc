...
addin_lines = [
    ...
    ("CAN USE on a SQL warehouse, and SELECT on the data.", False),
    ("", False),
    ("WHERE TO GET THE ADD-IN", True),
    ("  Easiest - Microsoft Marketplace (if your tenant allows store add-ins):", False),
    ("    In Excel: Home > Add-ins > More Add-ins (Store) > search 'Databricks Add-in for Excel' > Add", False),
    ("    (Marketplace product ID: 305c35cc-5f58-46b6-8701-d611c3379a86)", False),
    ("  Manual - download the manifest XML from the setup doc page (links under 'Install the add-in'):", False),
    ("    AWS/GCP workspaces: https://docs.databricks.com/aws/en/integrations/excel-setup", False),
    ("    Azure workspaces:   https://learn.microsoft.com/en-us/azure/databricks/integrations/excel-setup", False),
    ("    Pick the multi-workspace manifest (recommended); this XML is the 'add-in file'", False),
    ("    referenced in the install steps below.", False),
    ("  IT central deploy: Microsoft 365 admin center > Integrated apps > search the same name;", False),
    ("    then allowlist workspace URLs via PowerShell:", False),
    ("    Set-OrganizationAddInOverrides -ProductId 305c35cc-5f58-46b6-8701-d611c3379a86", False),
    ("      -AppDomains 'https://<workspace-url>'", False),
    ("", False),
